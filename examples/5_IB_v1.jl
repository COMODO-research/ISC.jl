...
using Printf
### To Do:  Shift it in the Main Function

function shift_faces(F, offset)
    return [typeof(f)(ntuple(i -> f[i] + offset, length(Tuple(f)))...) for f in F]
end
...
VWeb_E1      =      [Point{3, Float64}(v[1], v[2], v[3]-depthWeb+tFL) for v in VWeb_S1]
...
### ### ### MAKE ALL ONE PART
V           =         vcat(VFL_T, VWeb2, VFL_B)
# F           =         vcat(
#                         FFL_T,
#                         shift_faces(FWeb2, length(VFL_T)),
#                         shift_faces(FFL_B, length(VFL_T) + length(VWeb2))
#                     )

E           =          vcat(EFL_T, 
                        shift_elements(EWeb2,length(VFL_T)),
                        shift_elements(EFL_B,length(VFL_T)+length(VWeb2)))
F           =           element2faces(E)
...
# scatter!(ax1, VWeb2     , markersize = markersize *0.50   , color = :Pink)
# scatter!(ax1, VFL_2     , markersize = markersize *0.50   , color = :Blue)
# scatter!(ax1, VWeb_E1   , markersize = markersize *0.50   , color = :red)
...
poly!(ax1, GeometryBasics.Mesh(V, F), strokewidth = linewidth, color = :white, strokecolor = :black, shading = FastShading, transparency = false)
# normalplot(ax1,F,V)

# wireframe!(ax1, GeometryBasics.Mesh(Vh, Eh), linewidth = 3, color = :red)
fig

file1 = "merged_mesh.inp"
open(file1,"w") do io
    println(io, "*Heading")
